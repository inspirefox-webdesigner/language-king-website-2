...
export const getAllHomeTestimonials = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to fetch home testimonials", details: error.message });
  }
};
...
export const getHomeTestimonialById = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to fetch home testimonial", details: error.message });
  }
};

export const createHomeTestimonial = async (req, res) => {
  try {
    const { video_url, thumbnail, text, avatar, name, subtitle, time, rating } = req.body;
    const [result] = await db.execute(
      "INSERT INTO home_testimonials (video_url, thumbnail, text, avatar, name, subtitle, time, rating) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
      [video_url || "", thumbnail || "", text || "", avatar || "", name || "", subtitle || "", time || "", rating || 5]
    );
    res.status(201).json({
      id: result.insertId,
    ...
    res
      .status(500)
      .json({ error: "Failed to create home testimonial", details: error.message });
  }
};
...
export const updateHomeTestimonial = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to update home testimonial", details: error.message });
  }
};

export const deleteHomeTestimonial = async (req, res) => {
  try {
    await db.execute("DELETE FROM home_testimonials WHERE id = ?", [req.params.id]);
    res.json({ message: "Home testimonial deleted successfully" });
  } catch (error) {
    console.error("Error deleting home testimonial:", error);
    res
      .status(500)
      .json({ error: "Failed to delete home testimonial", details: error.message });
  }
};
