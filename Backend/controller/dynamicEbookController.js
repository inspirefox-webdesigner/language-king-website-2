...
export const getAllDynamicPages = async (req, res) => {
  try {
    ...
    const pagesWithUrl = pages.map((page) => ({
      ...page,
      url: `${frontendUrl}/ebook/${page.page_slug}`,
    }));

    res.json(pagesWithUrl);
  } catch (error) {
...
export const createDynamicPage = async (req, res) => {
  try {
    ...
    const [result] = await db.query("INSERT INTO dynamic_ebook_pages SET ?", [
      pageData,
    ]);

    // Generate frontend URL
    const frontendUrl = process.env.FRONTEND_URL || "http://localhost:5174";
    const pageUrl = `${frontendUrl}/ebook/${pageData.page_slug}`;
