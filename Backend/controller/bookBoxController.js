...
const getBookBox = async (req, res) => {
  try {
    const query =
      "SELECT * FROM book_box ORDER BY updated_at DESC, id DESC LIMIT 1";
    ...
    if (rows.length === 0) {
      ...
      await db.execute(insertQuery, [
        defaultData.ebook_title,
        defaultData.ebook_subtitle,
        defaultData.ebook_price,
        defaultData.aiportal_title,
        defaultData.aiportal_subtitle,
        defaultData.aiportal_price,
        defaultData.aiportal_original_price,
        defaultData.aiportal_features,
        defaultData.aiportal_image,
        defaultData.prediction_file_title,
        defaultData.prediction_file_subtitle,
        defaultData.prediction_file_price,
      ]);
...
const updateBookBox = async (req, res) => {
  try {
    ...
    res.json({
      message:
        "Book-Box updated successfully! Changes are now live on the website.",
      success: true,
...
export { getBookBox, updateBookBox };
