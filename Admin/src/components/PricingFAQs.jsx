...
const PricingFAQs = () => {
  ...
  const fetchFAQs = async () => {
    try {
      ...
      showToast("Error fetching pricing FAQs", "error");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const url = editingFaq
        ? `${API_BASE_URL}/pricing-faqs/${editingFaq.id}`
        : `${API_BASE_URL}/pricing-faqs`;
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/pricing-faqs/${id}`, {
        method: "DELETE",
      });
  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold">Pricing Page FAQs</h1>
        <button
